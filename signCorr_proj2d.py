...
def projection(data,ev,neg_flag):
    #ev : row vector 
    if neg_flag==False:
        proj_x = np.dot(data,ev)
    else:
        proj_x = np.dot(data,-ev)
    proj_x_points = np.expand_dims(proj_x,axis=-1)*np.expand_dims(ev,axis=0) #nxd
    return proj_x_points

def mse(data,proj_points):
    error = ((data - proj_points)**2).mean(axis=None)
    return error

def sign_correction(data,evs_ori):
    evs=evs_ori.T #make it row vector
    
    #first axis
    #+ve
    projected_points = projection(data,evs[0],False)
    mse_pos = mse(data,projected_points)
    #-ve
    projected_points = projection(data,evs[0],True)
    mse_neg = mse(data,projected_points)
    print("1,+ve : ",mse_pos )
    print("1,-ve : ",mse_neg )
    if mse_pos>mse_neg:
        #make correction
    ...
    #second axis
    #+ve
    projected_points = projection(data,evs[1],False)
    mse_pos = mse(data,projected_points)
    #-ve
    projected_points = projection(data,evs[1],True)
    mse_neg = mse(data,projected_points)
    print("2,+ve : ",mse_pos )
    print("2,-ve : ",mse_neg )
    if mse_pos>mse_neg:
        ...
        print("Correction 2")
        evs[1]=-evs[1]
    return evs.T #make it again column vector
...
if __name__=='__main__':
    ...
    rotation=120 #degrees +ve: counter clockwise
    ...
    X_s = sign_correction(df_src_scaled,X_s) #src
    X_t = sign_correction(df_target_scaled,X_t) #target
